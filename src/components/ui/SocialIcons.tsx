...
/**
 * The solid WhatsApp glyph (the SVG Repo mark), as opposed to the outlined
 * `WhatsAppIcon` above.
 *
 * It is drawn on a 48-unit grid with the artwork offset far off-origin, so the
 * translate on the group is part of the mark and cannot be dropped. `fill` is
 * `currentColor` rather than the hard-coded green it ships with, so the one
 * colour definition in `CHANNEL_COLORS` still governs it.
 */
export function WhatsAppSolidIcon({ size = 24, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      ...
      aria-hidden="true"
    >
      <g fill="currentColor" fillRule="evenodd" transform="translate(-700, -360)">
...
/**
 * A handset, drawn in its own two colours rather than `currentColor`.
 *
 * Unlike the single-colour marks above this one is inherently two-tone — a dark
 * body with a pale screen — so it cannot be tinted from the outside. The paint
 * order matters: the screen is drawn last and sits over the two body caps,
 * which is what gives the bezel its shape.
 */
export function MobileIcon({ size = 24, className }: IconProps) {
  return (
    ...
    >
      <path
        fill="#06344C"
        d="M354.121,0H157.884c-19.705,0-35.68,15.974-35.68,35.68l133.798,22.756L389.8,35.68 C389.8,15.974,373.826,0,354.121,0z"
      />
      <path
        fill="#06344C"
        d="M122.204,476.32c0,19.705,15.974,35.68,35.68,35.68h196.237c19.705,0,35.68-15.974,35.68-35.68 l-133.798-22.756L122.204,476.32z"
      />
      <rect x="122.2" y="35.68" width="267.594" height="440.639" fill="#99dcf5" />
    </svg>
  )
}

/**
 * The Gmail app mark, in Google's four colours.
 *
 * Self-coloured like `MobileIcon`, so the channel colour beside it only drives
 * the arrow and hover rule. The white path is the app tile it ships with — it
 * disappears against a light panel, leaving the coloured envelope, which is the
 * part that reads.
 */
...
/**
 * Official brand colours, for marks rendered in colour rather than mono.
 *
 * Kept beside the paths so a mark and its colour cannot drift apart, and so
 * every surface that renders these icons reaches for the same values. Phone and
 * Email are not social networks, but they sit in the same channel rows and need
 * the same treatment — a recognisable colour per channel is what makes the row
 * scannable without reading the labels.
 */
...
export type ChannelName = keyof typeof CHANNEL_COLORS
